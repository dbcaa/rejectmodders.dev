...
export function ContactSection() {
  ...
  return (
    <section ref={ref} id="contact" className="relative py-16 sm:py-24 md:py-32 scroll-mt-20" style={{ overflow: "clip" }}>
      {/* Ambient orbs */}
      <div className="pointer-events-none absolute left-1/2 top-1/2 -z-10 h-[400px] w-[400px] -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl"
        style={{ background: "radial-gradient(circle, oklch(0.45 0.18 20 / 0.08) 0%, transparent 70%)" }} />
      <div className="pointer-events-none absolute right-0 top-1/4 -z-10 h-64 w-64 rounded-full blur-3xl"
      ...
      <div className="mx-auto max-w-5xl px-4">
        ...
        >
          ...
          <h2 className="mt-2 text-3xl font-bold text-foreground sm:text-4xl md:text-5xl text-balance">{"Let's Work Together"}</h2>
          <motion.div
          ...
          />
          <p className="mt-4 text-muted-foreground max-w-lg mx-auto leading-relaxed">Whether it's a project collab, security stuff, or you just want to chat, I'm usually around.</p>
        </motion.div>
        ...
        >
          ...
          <div className="relative grid gap-8 md:grid-cols-2 md:gap-10">
            ...
            <div className="flex flex-col items-center md:items-start">
              {stats?.avatar_url && (
                <div className="relative mb-4">
                  <motion.div animate={{ scale: [1, 1.08, 1], opacity: [0.5, 0.15, 0.5] }}
            ...
            <div className="flex flex-col gap-3">
              <p className="mb-2 font-mono text-xs text-primary/70 hidden md:block">// get in touch</p>
              {links.map((link, i) => {
                const isFirst = i === 0
                const sharedClass = `group flex items-center gap-4 rounded-xl px-5 py-4 font-medium border transition-all duration-200 ${
                  isFirst
                    ? "border-primary bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5"
                    : "border-border bg-card hover:border-primary/30 hover:bg-card/80 hover:-translate-y-0.5"
                }`
                const inner = (
                  <>
