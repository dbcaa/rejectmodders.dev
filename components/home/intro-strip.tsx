...
import { Shield, Code2, MapPin, Zap, Github, Star, GitFork, Terminal, Lock, ArrowRight, GitCommit, GitPullRequest, AlertCircle, Clock, Server } from "lucide-react"
...
const TOOLS = [
  ...
  "VulnRadar", "Cybersecurity", "Discord Bots", "Open Source", "Web Hosting",
]
...
const FOCUS_CARDS = [
  {
    ...
    desc: "I got into security because I wanted to understand how things break. VulnRadar came out of that - a scanner that actually tells me something useful.",
  ...
  },
  {
    ...
    desc: "I go back and forth between C/C++ for low-level stuff and Next.js for web. Stack doesn't matter to me - I pick whatever gets the job done cleanly.",
    link: "/projects",
  ...
  },
  {
    ...
    tag: "// hosting",
    title: "Web Hosting",
    desc: "Started a hosting company through Disutils Team, and in 2026 launched WSLATL LLC. It's been growing steadily since launch.",
    link: "https://github.com/wslatl",
    linkLabel: "wslatl on github",
  },
  {
    icon: Lock,
    tag: "// open source",
    title: "Building in Public",
    desc: "All my stuff is open source. For security tools especially - if you can't read the code, why would you trust it? Fork it, poke holes in it.",
...
export function IntroStrip() {
  ...
  const noActivity = !eventsLoading && events.length === 0
  const recentRepos = useRecentRepos(noActivity)
  ...
  const statCards = [
    ...
    { icon: Shield,  label: "Vuln Checks",   value: 310,              suffix: "+", static: true },
  ]
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {[
              ...
              { icon: Server,  label: "Hosting",        value: "WSLATL LLC · 2026",       mono: true  },
            ].map((card, i) => (
              <motion.div
                key={card.label}
                initial={{ opacity: 0, y: 16 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: DUR, delay: 0.05 + i * SCROLL_STEP, ease: EASE }}
                className="flex items-start gap-3 rounded-xl border border-primary/40 bg-primary/5 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/60"
              >
      ...
      <div ref={actRef} className="py-10 md:py-14 bg-card/30">
        <div className="mx-auto max-w-6xl px-4">
          <div className="flex items-end justify-between mb-6">
            ...
            <motion.div initial={{ opacity: 0 }} animate={actInView ? { opacity: 1 } : {}} transition={{ duration: DUR, delay: 0.2 }}>
              <a
          ...
          {eventsLoading ? (
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {events.map((e, i) => (
                <motion.a
                  ...
                  animate={actInView ? { opacity: 1, y: 0 } : {}}
                  transition={{ duration: DUR, delay: 0.05 + i * SCROLL_STEP, ease: EASE }}
                  className="group flex items-start gap-3 rounded-xl border border-border bg-card p-3.5 transition-all duration-200 hover:border-primary/30 hover:-translate-y-0.5"
                >
            ...
            <>
              <p className="mb-3 font-mono text-xs text-muted-foreground/50">No recent public events — showing latest repos instead</p>
              <div className="grid gap-2 sm:grid-cols-2">
                {recentRepos.map((repo, i) => (
                  <motion.a
                    ...
                    className="group flex items-start gap-3 rounded-xl border border-border bg-card p-3.5 transition-all duration-200 hover:border-primary/30 hover:-translate-y-0.5"
                  >
                    <div className="mt-0.5 shrink-0"><Github className="h-3.5 w-3.5 text-muted-foreground" /></div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-mono text-[10px] text-primary/70">{repo.full_name}</p>
                      <p className="mt-0.5 truncate text-xs text-foreground">{repo.description ?? "No description"}</p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0 text-[10px] text-muted-foreground/50 font-mono">
                      <Clock className="h-2.5 w-2.5" />
            ...
            <motion.div
              ...
              animate={actInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: DUR, ease: EASE }}
              className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card/50 py-12 text-center"
            >
              <Github className="h-8 w-8 text-muted-foreground/30" />
              <p className="font-mono text-sm text-muted-foreground/60">No recent public activity found.</p>
              <a
                href="https://github.com/RejectModders"
                target="_blank" rel="noopener noreferrer"
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: DUR, delay: SCROLL_STEP, ease: EASE }} style={{ originX: 0 }}
